...
<form action="{{ route("admin.$relatedTo.resource.update.local",[$$relatedTo->unique_id,$resource->unique_id]) }}" method="post" enctype="multipart/form-data" id="form">
    @csrf 
        ...
        <div id="local-tab" class="tab-pane fade in active">
            ...
            <section class="local_start_wrapper">
                ...
                <div class="col-sm-12 artcover">
                    ...
                    <a href="#" data-toggle="modal" data-target="#modal-productPopup">
                        @if(isset($resource->product->unique_id)) 
                        Attach
                        @else
                        Change
                        @endif
                ...
                @php
                $product = asset(config('constants.media.default_media_path_display'));
                $filename = '';
                if(isset($resource->product->media->file)):
                $filename = $resource->product->media->file; 
                $product = asset(config('constants.media.media_path_display').$resource->product->media->file);
                endif;
                @endphp
                <div class="col-sm-12 imagewrap">
                    <div class="col-sm-3 covercontainer">
                        <img src="{{ $product }}" alt="Cover Image" id="product_preview" class="image" style="width:100%">
                        <div class="middle">
                            <div class="text product_name">{{ $filename }}</div>
                        </div>
                    </div>
                    <input type="hidden" name="product_id" value="{{ @$resource->product->media_id }}"/>
                    <div class="col-sm-9 editimg product_detail" @if(!$filename) {{ 'style=display:none;' }} @endif>
                         <div class="postnbotm">
                            <h5 class="product_name">{{ $filename }}</h5>
                            @if(isset($resource->product->media_id))
                            @endif
                        </div>
                    </div>   
                </div>
