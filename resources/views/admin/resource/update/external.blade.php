...
<form action="{{ route("admin.$relatedTo.resource.update.external",[$$relatedTo->unique_id,$resource->unique_id]) }}" method="post" enctype="multipart/form-data" id="form">
    @csrf 
    <div class="tab-content">
        <div id="external-tab" class="tab-pane fade in active">
            ...
            <section class="local_start_wrapper">
                ...
                <div class="col-sm-12 artcover">
                    ...
                    <a href="#" data-toggle="modal" data-target="#modal-externalPopup">
                        @if(isset($resource->external->unique_id)) 
                        Attach
                        @else
                        Change
                        @endif
                ...
                @php
                $external = asset(config('constants.media.default_media_path_display'));
                $filename = '';
                if(isset($resource->external->media->file)):
                $filename = $resource->external->media->file; 
                $external = asset(config('constants.media.media_path_display').$resource->external->media->file);
                endif;
                @endphp
                <div class="col-sm-12 imagewrap">
                    <div class="col-sm-3 covercontainer">
                        <img src="{{ $external }}" alt="Cover Image" id="external_preview" class="image" style="width:100%">
                        <div class="middle">
                            <div class="text external_name">{{ $filename }}</div>
                        </div>
                    </div>
                    <input type="hidden" name="external_id" value="{{ @$resource->external->media_id }}"/>
                    <div class="col-sm-9 editimg external_detail" @if(!$filename) {{ 'style=display:none;' }} @endif>
                         <div class="postnbotm">
                            <h5 class="external_name">{{ $filename }}</h5>
                            @if(isset($resource->external->media_id))
                            @endif
                        </div>
                    </div>   
                </div>
        ...
        <div id="cover-tab" class="tab-pane fade in">
            <div class="admin-nav-head">The cover represents the external. Add the settings below to control what the cover looks like.</div>
            <div class="col-sm-12 artctitle">
                <h5>Cover Title: If you don't want the label of the cover to be the same as the title of the external, you may input something different here instead.</h5>
                <div class="form-group">
            ...
            <div class="col-sm-12 artcover">
                ...
                <a href="#" data-toggle="modal" data-target="#modal-coverPopup">
                    @if(isset($resource->cover_external->unique_id)) 
                    Attach
                    @else
                    Change
                    @endif
