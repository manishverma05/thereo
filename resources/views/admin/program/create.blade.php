...
<!-- Start Navigation -->
<form action="{{ route('admin.program.create') }}" method="post" enctype="multipart/form-data" id="form">
    @csrf 
    <!-----Article Add Section------>
    <input type="hidden" name="status" value="{{ old('status') }}"/>
    <div class="tab-content">
        <div id="progdetail-tab" class="tab-pane fade in active">
            <div class="admin-nav-head">Add this program using the settings below.</div>
        ...
        <div id="cover-tab" class="tab-pane fade in">
            <div class="admin-nav-head">The cover represents the program. Add the settings below to control what the cover looks like.</div>
            <div class="col-sm-12 artctitle">
            ...
            <div class="col-sm-12 meta-auther userAccess">
                ...
                <div class="auth_wrap">
                    ...
                    <label class="auth_container">{{ $role->name }}
                        <input type="radio"  name="role_id[]" value="{{ $role->id }}"  @if(is_array(old('role_id')) && in_array($role->id, old('role_id'))) checked @elseif($role->id == 1 ) checked @endif >
                               <span class="checkmark"></span>
                    </label>
                    @endforeach
                </div>
